...
This app has the ability to livereload when changes are made to the handlers. To use this feature, run the following command:

```bash
my-application$ npm run watch
```

What this will do is first lint and format all files in the `src` folder, then watch for all changes in the `src` folder, if the change occurs directly on the function handler, only that handler will be rebuilt and then copied over to the local `.aws-sam/build` folder. If the change occurs on any other file, all handlers will be rebuilt and copied over to the local `.aws-sam/build` folder. The files responsible for this are `lambdaBuild` (which does the compilation for the handlers) and `lambdaBuildLivereload` (which watches and lints files durring development). This provided as a developer experience helper, and does not replace running `sam build` when going to production.
...
```bash
sam delete --stack-name instructor-portal-backend
```
...
To manually lint and format your code, use:
